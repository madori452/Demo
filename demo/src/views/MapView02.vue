        ...
        .then((res) => {
          ...
          this.stopData.forEach(e => {
            ...
            if (e.type === '臨時站牌') {
              ...
              const vectorStopLayer = new VectorLayer({
                source: vectorSource
              })
              this.mapInit.addLayer(vectorStopLayer)
            }
            // 點選功能
            this.mapInit.on('click', ev => {
              ...
              if (feature) {
                ...
                content.innerHTML = '<p>' + '站點名稱：' + featureStaion.get('name') + '</p>'
                content.innerHTML += '<p>' + '站點類型：' + featureStaion.get('type') + '</p>'
              }
            })
          })
        }).catch(err => {
